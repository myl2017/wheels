...
export default {
  ...
  computed: {
    areAllItemsSelected() {
      const a = this.dataSoruce.map(item => item.id).sort()
  ...
  watch: {
    selectedItems() {
      if (this.selectedItems.length === this.dataSoruce.length) {
  ...
  methods: {
    ...
    onChangeAllItems(e) {
      let selected = e.target.checked
      this.$emit('update:selectedItems', selected ? this.dataSoruce : [])
    }
  }
}
</script>
